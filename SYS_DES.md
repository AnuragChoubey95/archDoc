...
### 5.5 Client Connection Failures

Clients may fail to establish a TCP connection to the router (network outage, router down, SYN drops, etc.).  
Since the router has no visibility before `accept()` succeeds, connection establishment is entirely a client-side responsibility.  
Clients are expected to implement retry with backoff and reconnect to any available router instance.  
The router design begins only after a connection is successfully accepted.
